...
export const WithApiBearerAuthRequired = (
            ...
            if (scope !== undefined) {
                if (!claims.scope) {
                    return res.status(403).json({
                        error: "access_denied",
                        error_description:
                            "Token does not contain a scope, which is required for this call",
                    });
                } else {
                    ...
                    if (!scopeFound) {
                        return res.status(403).json({
                            error: "access_denied",
                            error_description:
                                "Token contains a scope, but not one that is required for this call",
                        });
                    }
                }
            }
